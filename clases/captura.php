...
if(is_array($datos)){
    ...
    //$email = $datos['detalles']['payer']['email_address']; lo sacamos del cliente de la sesion
    //$id_cliente = $datos['detalles']['payer']['payer_id']; lo sacamos ahora del cliente
    $email = $row_cliente['email'];
    



    $sql = $com->prepare("INSERT INTO compra(id_transaccion, fecha, status, email, id_cliente, total) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    if($id > 0){
        ...
        if($productos != null){
            foreach($productos as $clave => $cantidad){
                ...
                $sql_insert = $com->prepare("INSERT INTO  detalle_compra(id_compra, id_producto, nombre, precio, cantidad) VALUES (?, ?, ?, ?, ?)");
